...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def __init__(self, filenames: Sequence[str], root: Optional[str] = None):
    ...
    def process_one(self, filename: str):
        """Analyze the specified Python source file."""

        if filename not in self.filenames:
            # TODO: Clearer message.
            raise ValueError(
                f"Filename {filename} has not been preprocessed "
                f"(was not given to __init__, which got {self.filenames})."
            )
    ...
    def resolve_base_classes(self):
        ...
        """
        logger.debug("Resolving base classes")
    ...
    def visit_Module(self, node):
        ...
        self.name_stack.append(namespace)
        self.scope_stack.append(self.scopes[namespace])
        self.context_stack.append("Module %s" % (namespace))
    ...
    def visit_FunctionDef(self, node):
        logger.debug("FunctionDef %s, %s:%s" % (node.name, self.filename, node.lineno))
        ...
        if self_name is not None:
            ...
            self.scopes[inner_namespace].defs[self_name] = class_node
            logger.info('Method def: setting self name "%s" to %s' % (self_name, class_node))
    ...
    def generate_args_nodes(self, ast_args, inner_namespace):
        """Capture which names correspond to function args.
        ...
        inner_namespace: namespace of the function or lambda, for scope lookup
        """
        scope = self.scopes[inner_namespace]
    ...
    def visit_Import(self, node):
        logger.debug("Import %s, %s:%s" % ([format_alias(x) for x in node.names], self.filename, node.lineno))
    ...
    def visit_ImportFrom(self, node):
        ...
        for alias in node.names:
            ...
            logger.info("From setting name %s to %s" % (alias_name, to_node))

            logger.debug("Use from %s to ImportFrom %s" % (from_node, to_node))
            if self.add_uses_edge(from_node, to_node):
                logger.info("New edge added for Use from %s to ImportFrom %s" % (from_node, to_node))
    ...
    def analyze_module_import(self, import_item, ast_node):
        ...
        logger.info("New edge added for Use import %s in %s" % (mod_node, from_node))
        self.set_value(alias_name, mod_node)  # set node to be discoverable in module
        logger.info("From setting name %s to %s" % (alias_name, mod_node))
        ...
        if isinstance(node.ctx, ast.Store):
            ...
            try:
                if self.set_attribute(node, new_value):
                    logger.info("setattr %s on %s to %s" % (node.attr, objname, new_value))
    ...
    def visit_Name(self, node):
        logger.debug("Name %s in context %s, %s:%s" % (node.id, type(node.ctx), self.filename, node.lineno))
    ...
    def visit_Assign(self, node):
        ...
        if len(node.targets) > 1:
            logger.debug("Assign (chained with %d outputs)" % (len(node.targets)))
    ...
    def visit_For(self, node):
        logger.debug("For-loop, %s:%s" % (self.filename, node.lineno))
    ...
    def visit_ListComp(self, node):
        logger.debug("ListComp, %s:%s" % (self.filename, node.lineno))
        self.analyze_comprehension(node, "listcomp")

    def visit_SetComp(self, node):
        logger.debug("SetComp, %s:%s" % (self.filename, node.lineno))
        self.analyze_comprehension(node, "setcomp")

    def visit_DictComp(self, node):
        logger.debug("DictComp, %s:%s" % (self.filename, node.lineno))
        self.analyze_comprehension(node, "dictcomp", field1="key", field2="value")

    def visit_GeneratorExp(self, node):
        logger.debug("GeneratorExp, %s:%s" % (self.filename, node.lineno))
    ...
    def visit_Call(self, node):
        logger.debug("Call %s, %s:%s" % (get_ast_node_name(node.func), self.filename, node.lineno))
    ...
    def visit_With(self, node):
        logger.debug("With (context manager), %s:%s" % (self.filename, node.lineno))
    ...
    def resolve_builtins(self, ast_node):
        ...
        if isinstance(func_ast_node, ast.Name):
            ...
            if funcname == "super":
                class_node = self.get_current_class()
                logger.debug("Resolving super() of %s" % (class_node))
                if class_node in self.mro:
                    ...
                    if len(self.mro[class_node]) > 1:
                        ...
                        logger.debug("super of %s is %s" % (class_node, result))
                        return result
                    else:
            ...
            if funcname in ("str", "repr"):
                if len(ast_node.args) == 1:  # these take only one argument
                    ...
                    if isinstance(obj_astnode, (ast.Name, ast.Attribute)):
                        logger.debug("Resolving %s() of %s" % (funcname, get_ast_node_name(obj_astnode)))
    ...
    def resolve_attribute(self, ast_node):
        ...
        if isinstance(ast_node.value, ast.Attribute):
            ...
            if isinstance(obj_node, Node) and obj_node.namespace is not None:
                namespace = obj_node.get_name()  # fully qualified namespace **of attr**
                if namespace in self.scopes:  # imported modules not in the set of analyzed files are not seen by Pyan
                    scope = self.scopes[namespace]
                    if attr_name in scope.defs:
                        logger.debug("Resolved to attr %s of %s" % (ast_node.attr, scope.defs[attr_name]))
                        return scope.defs[attr_name], ast_node.attr
            ...
            #
            logger.debug("Unresolved, returning attr %s of unknown" % (ast_node.attr))
            ...
            # attribute of a function call. Detect cases like super().dostuff()
                ...
                if not isinstance(obj_node, Node):
                    logger.debug("Unresolved function call as obj, returning attr %s of unknown" % (ast_node.attr))
                    return None, ast_node.attr
            else:
                # Get the Node object corresponding to node.value in the current namespace.
                #
                # (Using the current namespace here is correct; this case only gets
                #  triggered when there are no more levels of recursion,
                #  and the leftmost name always resides in the current namespace.)
                obj_node = self.get_value(get_ast_node_name(ast_node.value))  # resolves "self" if needed

        logger.debug("Resolved to attr %s of %s" % (ast_node.attr, obj_node))
        return obj_node, ast_node.attr

    ###########################################################################
    # Scope analysis

    def analyze_scopes(self, code: str, filename: str):
        """Gather lexical scope information."""
        # namespace is the fully qualified ("dotted") name of scope.
        ...
        # TODO: Test and benchmark recursive_scope_v*
        scopes = recursive_scope_v1(
            self.module_name,
        ...
        for namespace, scope in scopes.items():
            if namespace not in self.scopes:  # add new scope info
                self.scopes[namespace] = scopes[namespace]
            else:  # update existing scope info
                old_scope = self.scopes[namespace]
                for name in scope.defs:
                    if name not in old_scope.defs:
                        old_scope.defs[name] = scope.defs[name]

        logger.debug(f"Scopes now: {self.scopes}")
    ...
    def get_value(self, name):
        ...
        def find_scope(name):
            for scope in reversed(self.scope_stack):
                if name in scope.defs and scope.defs[name] is not None:
                    return scope

        scope = find_scope(name)
        if scope is not None:
            value = scope.defs[name]
            if isinstance(value, Node):
                logger.info("Get %s in %s, found in %s, value %s" % (name, self.scope_stack[-1], scope, value))
                return value
            else:
                # TODO: should always be a Node or None
                logger.debug(
                    "Get %s in %s, found in %s: value %s is not a Node" % (name, self.scope_stack[-1], scope, value)
                )
        else:
            logger.debug("Get %s in %s: no Node value (or name not in scope)" % (name, self.scope_stack[-1]))
    ...
    def set_value(self, name, value):
        ...
        if scope is not None:
            if isinstance(value, Node):
                ...
                logger.info("Set %s in %s to %s" % (name, scope, value))
            else:
                # TODO: should always be a Node or None
                logger.debug("Set %s in %s: value %s is not a Node" % (name, scope, value))
        else:
            logger.debug("Set: name %s not in scope" % (name))
        ...
        if isinstance(obj_node, Node) and obj_node.namespace is not None:
            ...
            def lookup(namespace):
                if namespace in self.scopes:
                    scope = self.scopes[namespace]
                    if attr_name in scope.defs:
                        return scope.defs[attr_name]

            # first try directly in object's namespace (this works already in pass 1)
            value_node = lookup(namespace)
            if value_node is not None:
                return obj_node, value_node

            # next try namespace of each ancestor (this works only in pass 2,
    ...
    def set_attribute(self, ast_node, new_value):
        ...
        if isinstance(obj_node, Node) and obj_node.namespace is not None:
            ...
            if namespace in self.scopes:
                scope = self.scopes[namespace]
                scope.defs[attr_name] = new_value
                return True
        return False
    ...
    def create_node(self, namespace, name, ast_node=None, flavor=Flavor.UNSPECIFIED):
        ...
        *IMPORTANT* In CallGraphVisitor, always use create_node() to create nodes,
        ...
        node = Node(namespace, name, ast_node, filename, flavor)
    ...
    def remove_wild(self, from_node, to_node, name):
        ...
        if len(matching_wilds):
            ...
            logger.info("Use from %s to %s resolves %s; removing wildcard" % (from_node, to_node, wild_node))
        ...
        for n in self.uses_edges:
            for n2 in self.uses_edges[n]:
                if n2.namespace is not None and not n2.defined:
                    ...
                    logger.info("Contracting non-existent from %s to %s as %s" % (n, n2, n3))
    ...
    def expand_unknowns(self):
        ...
        for from_node, to_node in new_defines_edges:
            ...
            logger.info("Expanding unknowns: new defines edge from %s to %s" % (from_node, to_node))
        ...
        for from_node, to_node in new_uses_edges:
            self.add_uses_edge(from_node, to_node)
            logger.info("Expanding unknowns: new uses edge from %s to %s" % (from_node, to_node))
        ...
        for n in self.uses_edges:
            for n2 in self.uses_edges[n]:
                ...
                if inherited and n in self.uses_edges:
                    removed_uses_edges.append((n, n2))
                    logger.info("Removing inherited edge from %s to %s" % (n, n2))
                ...
                for n in self.nodes[name]:
                    ...
                    if n in self.uses_edges:
                        for n2 in self.uses_edges[n]:  # outgoing uses edges
                            logger.info("Collapsing inner from %s to %s, uses %s" % (n, pn, n2))
